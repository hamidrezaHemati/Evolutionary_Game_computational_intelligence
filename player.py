...
class Player():
    ...
    def think(self, mode, box_lists, agent_position, velocity):
        ...
        vertical_distances = []  # vertical distance between agent and next 2 boxes   ---  عمودی
        horizontal_distances = []  # horizontal distance between agent and next 2 boxes  --- افقی

        if len(box_lists) == 0:
            horizontal_distances.append(CONFIG['WIDTH'])
            horizontal_distances.append(CONFIG['WIDTH'])
            vertical_distances.append(CONFIG['HEIGHT']/2)
            vertical_distances.append(CONFIG['HEIGHT']/2)
        elif len(box_lists) == 1:
            vertical_distances.append(agent_position[1] - box_lists[0].gap_mid)
            vertical_distances.append(CONFIG['HEIGHT'] / 2)
            horizontal_distances.append(agent_position[0] - box_lists[0].x)
            horizontal_distances.append(CONFIG['WIDTH'])
        else:
            for i in range(2):
                vertical_distances.append(agent_position[1] - box_lists[i].gap_mid)
                horizontal_distances.append(agent_position[0] - box_lists[i].x)

        # Normalization
        for i in range(2):
            vertical_distances[i] = vertical_distances[i]/CONFIG['HEIGHT']
            horizontal_distances[i] = horizontal_distances[i]/CONFIG['WIDTH']
        max_speed = 10

        input_layer = [velocity/max_speed, vertical_distances[0], vertical_distances[1], horizontal_distances[0],
                       horizontal_distances[1]]
